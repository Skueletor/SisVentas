        ...
        private void FrmProveedores_Load(object sender, EventArgs e)
        {
            ...
            foreach (Proveedor item in lista)
            {
                dgvdata.Rows.Add(new object[] {"",item.IdProveedor,item.Documento,item.RazonSocial,item.Correo,item.Telefono,
                    ...
                    item.Estado == true ? "1:Activo" : "2:No Activo"
                });
            }
        }
        ...
        private void btnbuscar_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(textoBusqueda))
            {
                foreach (DataGridViewRow row in dgvdata.Rows)
                {
                    ...
                    {
                        string valorCelda = row.Cells[columnaFiltro].Value.ToString().Trim().ToLower();
            ...
            {
                MessageBox.Show("Por favor, ingrese un término de búsqueda.", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
